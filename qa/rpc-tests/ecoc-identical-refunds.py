...
class EcocIdenticalRefunds(BitcoinTestFramework):
    def __init__(self):
        super().__init__()
    ...
    def run_test(self):
        ...
        self.node.sendtocontract(contract_address, "00", 0, 1000000, ECOC_MIN_GAS_PRICE_STR, sender_address)
        self.node.sendtoaddress(sender_address, 1)
        self.node.sendtocontract(contract_address, "00", 0, 1000000, ECOC_MIN_GAS_PRICE_STR, sender_address)
