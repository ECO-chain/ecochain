...
"""
class EcocDGPActivation(BitcoinTestFramework):
    def __init__(self):
        super().__init__()
    ...
    def run_test(self):
        ...
        # Activate a proposal for 128MB blocks
        max_block_size = 128000000
        ...
        # Submit a block close to 128MB and make sure that it was accepted
        ...
        # Activate a proposal for 16MB blocks
        ...
        # 1. blocksizelimit=128MB
        # 2. 128MB block submitted and accepted
        # 3. blocksizelimit=1MB
        # Now we should only allow new 1MB blocks,
        # however the old 128MB block not cause any errors when restarting since it was accepted at a time when 128MB was the block size limit

        # Restart the ecocd to verify that no crashes occurs on startup
        stop_nodes(self.nodes)
        self.setup_network()


if __name__ == '__main__':
    EcocDGPActivation().main()
