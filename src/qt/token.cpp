...
bool ToHash160(const std::string& strEcocAddress, std::string& strHash160)
{
    CBitcoinAddress ecocAddress(strEcocAddress);
    if(ecocAddress.IsValid()){
        CKeyID keyid;
        ecocAddress.GetKeyID(keyid);
...
bool ToEcocAddress(const std::string& strHash160, std::string& strEcocAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress ecocAddress;
    ecocAddress.Set(keyid);
    if(ecocAddress.IsValid()){
        strEcocAddress = ecocAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToEcocAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToEcocAddress(tokenEvent.receiver, tokenEvent.receiver);
