...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    ExtractQtumTX ecocTx;
    BOOST_CHECK(converter.extractionQtumTransactions(ecocTx));
    std::vector<QtumTransaction> result = ecocTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(ecoctxconverter_tests, TestingSetup)
