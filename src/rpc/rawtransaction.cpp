...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the ecoc address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
            ...
            "         \"amount\":x.xxx,                (numeric, optional) Value in Ecoc to send with the call, should be a valid amount, default 0\n"
    ...
    BOOST_FOREACH(const string& name_, addrList) {
        ...
        if (name_ == "data") {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Ecoc address: ")+name_);
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"Q2tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) ecoc address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
