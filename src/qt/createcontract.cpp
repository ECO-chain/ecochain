...
void CreateContract::setLinkLabels()
{
    ...
    ui->labelSolidity->setText(tr("<a href=\"https://ethereum.github.io/browser-solidity/\">Solidity compiler</a>"));

    ui->labelToken->setOpenExternalLinks(true);
    ui->labelToken->setText(tr("<a href=\"https://ethereum.org/token#the-code\">Token template</a>"));
}
...
void CreateContract::on_numBlocksChanged()
{
    if(m_clientModel)
    {
        ...
        ui->labelGasPrice->setToolTip(tr("Gas price: ECOC price per gas unit. Default = %1, Min = %2").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
