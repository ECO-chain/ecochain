...
{
    const std::string URL_SOURCE_CODE = "<https://gitlab.com/ecoc/ecochain>";
    const std::string URL_WEBSITE = "<https://ecoc.io>";
...
//////////////////////////////////////////////////////////////////////
    ...
    while (!fLoaded) {
		...
		dev::eth::Ethash::init();
                ...
                bool fStatus = boost::filesystem::exists(ecocStateDir);
                const std::string dirEcoc(ecocStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsEcocstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<QtumState>(new QtumState(dev::u256(0), QtumState::openDB(dirEcoc, hashDB, dev::WithExisting::Trust), dirEcoc, existsEcocstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::ecocMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(ecocStateDir.string());
