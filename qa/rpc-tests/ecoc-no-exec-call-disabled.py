...
class EcocNoExecCallDisabledTest(BitcoinTestFramework):
    def __init__(self):
        super().__init__()
    ...
    def run_test(self):
        ...
        tx.vout = [CTxOut(int(COIN), scriptPubKey=CScript([b"\x00", CScriptNum(100000), CScriptNum(ECOC_MIN_GAS_PRICE), b"\x00", hex_str_to_bytes(contract_address), OP_CALL]))]
        tx = rpc_sign_transaction(self.node, tx)
        assert_raises(JSONRPCException, self.node.sendrawtransaction, bytes_to_hex_str(tx.serialize()))
...
if __name__ == '__main__':
    EcocNoExecCallDisabledTest().main()
