...
bool WalletModel::isUnspentAddress(const std::string &ecocAddress) const
{
    LOCK2(cs_main, wallet->cs_wallet);
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if(fValidAddress && CBitcoinAddress(address).ToString() == ecocAddress && out.tx->tx->vout[out.i].nValue)
        {
            return true;
        }
    }
