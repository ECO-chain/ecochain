...
#if defined(NDEBUG)
# error "Ecoc cannot be compiled without assertions."
#endif
...
size_t nCoinCacheUsage = 500 * 300;
...
const std::string strMessageMagic = "Ecoc Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            std::vector<QtumTransaction> ecocTransactions = resultConverter.first;
            std::vector<EthTransactionParams> ecocETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(QtumTransaction ecocTransaction : ecocTransactions){
                sumGas += ecocTransaction.gas() * ecocTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, ecocTransaction.gasPrice());
                } else {
                    txMinGasPrice = ecocTransaction.gasPrice();
                }
                VersionVM v = ecocTransaction.getVersion();
                if(v.format!=0)
                ...
                if(ecocTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(ecocTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(ecocTransaction.gas() > UINT32_MAX)
            ...
            if(!CheckMinGasPrice(ecocETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > ecocTransactions.size())
...
bool CheckReward(const CBlock& block, CValidationState& state, int nHeight, const Consensus::Params& consensusParams, CAmount nFees, CAmount gasRefunds, CAmount nActualStakeReward, const std::vector<CTxOut>& vouts)
{
    ...
    if (block.IsProofOfWork()) {
        ...
        if (nPrevHeight >= consensusParams.nFirstMPoSBlock) {
            ...
            LogPrintf("number of cosigners to be rewarded: %d\n", rewardRecipients); //ecoclog
        }
        ...
        if (rewardRecipients == 1) {
            ecoc::ecocLog("First blocks: Reward goes fully to the winner node." );
            return true;
        }
...
{
    boost::filesystem::path ecocDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if (fIsVMlogFile) {
        ss << ",";
    }
    else {
        std::ofstream file(ecocDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(ecocDir.string(), std::ios::in | std::ios::out);
...
bool QtumTxConverter::extractionQtumTransactions(ExtractQtumTX& ecoctx)
{
    ...
    ecoctx = std::make_pair(resultTX, resultETP);
    return true;
}
...
//////////////////////////////////////////////////////////////////
    ...
    if (pindex->nHeight > ecoc::lastPoWBlock && !Params().GetConsensus().fPoSNoRetargeting) {
        ...
        uint64_t actualSupply = ecoc::getActualSupply(pindex->nHeight) * COIN;
        if (pindex->nMoneySupply > actualSupply) {
            LogPrintf("pindex->nMoneySupply: %u", uint64_t(pindex->nMoneySupply));
            LogPrintf("Reward limit: %u", uint64_t(actualSupply));
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Ecoc PoS we need to sync this after every block to ensure txdb is populated for
