...
double GetPoWMHashPS()
{
    ...
    return GetDifficulty() * 4294.967296 / nTargetSpacingWork; // 2^32 / 10^6 = 4294.967296
}
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress ecocSenderAddress(request.params[2].get_str());
        if(ecocSenderAddress.IsValid()){
            CKeyID keyid;
            ecocSenderAddress.GetKeyID(keyid);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of ecoc addresses\n"
