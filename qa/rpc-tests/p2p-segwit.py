...
class SegWitTest(BitcoinTestFramework):
    ...
    def test_block_malleability(self):
        ...
        assert(get_virtual_size(block) > MaxBlockBaseSize)
    ...
    def test_witness_block_size(self):
        ...
        i = 0
        while vsize < MaxBlockBaseSize+1:
            # Add some more bytes to each input until we hit MaxBlockBaseSize+1
            extra_bytes = min((MaxBlockBaseSize - vsize)*4-2, 5*55)
        ...
        block.solve()
        vsize = get_virtual_size(block)
        assert_equal(vsize, MaxBlockBaseSize + 1)
    ...
    def test_signature_version_1(self):
        ...
        for i in range(NUM_TESTS):
            ...
            if (get_virtual_size(block) > MaxBlockBaseSize - 1000):
                self.update_witness_block_with_transactions(block, [])
                self.test_node.test_witness_block(block, accepted=True)
                block = self.build_next_block()
